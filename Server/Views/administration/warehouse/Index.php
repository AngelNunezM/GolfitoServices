...
<main class="flex lg:flex-row xs:flex-col p-5">
    <header class="flex flex-col">
        <span class="text-cyan-300">Almacén</span>
        <h1 class="font-semibold text-4xl">Lista de productos</h1>
        <p class="text-tsecondary mt-2">Gestiona la lista de productos de tu negocio.</p>
    </header>
                    ...
                    <?php if (!empty($products)): ?>
                        <?php foreach ($products as $index => $product): ?>
                            <tr class="transition-colors hover:bg-gray-50/5">
                                ...
                                <td class="px-3 py-4 text-sm text-gray-400"><?= htmlspecialchars($product->name ?? '') ?></td>
                                <td class="px-3 py-4 text-sm text-gray-400"><?= htmlspecialchars((string) ($product->stock_min ?? 0)) ?></td>
                                <td class="px-3 py-4 text-sm text-gray-400">
                                    <span class="rounded-full px-2 py-1 text-xs <?= ($product->is_active ?? true) ? 'bg-emerald-500/20 text-emerald-300' : 'bg-red-500/20 text-red-300' ?>">
                                        <?= ($product->is_active ?? true) ? 'Activo' : 'Inactivo' ?>
                                    </span>
                                </td>
                                <td class="px-3 py-4 text-sm text-gray-300">
                                    <div class="flex gap-2">
                                        <a href="/administracion/almacen/<?= htmlspecialchars((string) ($product->id ?? '')) ?>/editar" class="rounded bg-cyan-500/20 px-2 py-1 text-xs text-cyan-200 hover:bg-cyan-500/30">Editar</a>
                                        <form method="POST" action="/administracion/almacen/<?= htmlspecialchars((string) ($product->id ?? '')) ?>/eliminar" onsubmit="return confirm('¿Deseas cambiar el estado del producto?')">
